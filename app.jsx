/* app.jsx — main composition, theme + tweaks wiring */
...
/* swatch palettes — first hex is bg, second is fg, third accent, fourth accent-2 */
const PALETTES = [
  ["#0e0e0c", "#f5f1e8", "#d97757", "#7a8f5c"], // warm dark (default)
  ["#f6f4ef", "#1a1916", "#d97757", "#7a8f5c"], // warm light
  ["#0a0e0a", "#e8ffd8", "#7dff9a", "#a39d8f"], // terminal green
  ["#0a0a0a", "#fafafa", "#ff5733", "#3d7dff"], // mono + electric
  ["#1a1f2e", "#e8d5b7", "#c97064", "#6b8e9f"], // dusk indigo
];
...
function applyTheme(t) {
  ...
  // simple "is light" check: average channel above 128
  const isLight = (() => {
    const h = bg.replace('#','');
  ...
  // derive surface tones from bg using channel offsets
  const off = (hex, d) => {
    const h = hex.replace('#','');
    const x = h.length === 3 ? h.replace(/./g, c => c+c) : h;
    const n = parseInt(x, 16);
